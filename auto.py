...
def getcity(*args):
    ct = city_entry.get()
    l1.delete(0, END)
    for i in [city for sublist in execute_query("select name from cities") for city in sublist]:
        if re.match(ct, i, re.IGNORECASE):
            l1.insert(END, i)
...
def customWeather():
    ...
    cus_city_name=city_entry.get()
    inpfield.delete("1.0", "end") 

    try:
        countryid, lat, lng = execute_query(f"select country_id, latitude, longitude from cities where name = '{cus_city_name}'")[0]
...
def showWeather():
    ...
    time_url = "https://www.timeapi.io/api/Time/current/coordinate?latitude="+str(lat)+"&longitude="+str(lng)
    weather_url = 'http://api.openweathermap.org/data/2.5/weather?q=' + show_city_name + '&appid='+api_key
    ...
    if weather_info['cod'] == 200:
        ...
        weather = f"Weather of: {show_city_name}, {country}\nTime : {time}\nTemperature (Celsius): {tempc}°C\nTemperature (Kelvin): {tempk}K\nTemperature (Farenheit) :{tempf}°F\nPressure: {pressure} hPa\nWind Speed: {wind_speed} m/s\nHumidity: {humidity}%\nSunrise at {sunrise_time} and Sunset at {sunset_time}\nCloud: {cloudy}%\nWeather Info: {description}" 
...
def background():
    try:
        img = Image.open(BytesIO(fetch("bg.jpg", "we", image=True)))
...
inpfield = Text(root, width=36, height=11, font="Arial 19",bg="BlanchedAlmond", highlightcolor = "BlanchedAlmond")
inpfield.place(x=width-555, y=int(height/3)+100)
city_entry = Entry(root, textvariable=city_value, width=25, font='Arial 16 bold')
city_entry.place(x=width-555, y=int(height/3)+67)
l1 = Listbox(root, width=25, height=1, font='Arial 16 bold')
l1.place(x=width-555, y=int(height/3)+95)
...
city_entry.bind("<KeyRelease>", getcity)
l1.bind("<<ListboxSelect>>", upd) 
showtime()
showWeather()
date()
root.mainloop()
